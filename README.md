...
- **每日自动构建**：每日北京时间凌晨 04:00 自动同步上游权威数据源（`Loyalsoldier`、`xishang0128` 等一手源）并自主清洗编译最新规则。
...
| 产物名称 | 说明 | 适用场景 | 预估体积 |
| :--- | :--- | :--- | :--- |
| **`geoip.rdb`** | **全球国家与专有标签全量数据库**（ISO 国家 IP + Telegram / Cloudflare 等专有网段） | 离线单文件全量首选 (PC/服务器) | **~8.6 MB** |
| **`geoip-lite.rdb`** | **精简国家与专有标签数据库**（核心国家 + 常用专有网段） | 软路由 / 低内存嵌入式设备首选 | **~380 KB** |
