...
- **复合数据库 `geoip.rdb` / `geoip-lite.rdb` 原生支持**：单个数据库同时包含全球 ISO 国家 IP、8 万个 ASN 自治域与 Telegram/Private/Cloudflare 等专有网段。
...
### 2. `release` 分支（离线大包与兼容数据库）

| 产物名称 | 说明 | 适用场景 | 预估体积 |
| :--- | :--- | :--- | :--- |
| **`geoip.rdb`** / `geoip.metadb` | **复合三合一全量数据库**（国家 + ASN + 专有标签） | 离线单文件全量首选 (PC/服务器) | ~8.4 MB |
| **`geoip-lite.rdb`** / `geoip-lite.metadb` | **复合三合一精简数据库**（核心国家 + 专有标签） | 软路由 / 低内存嵌入式设备首选 | **~386 KB** |
...
### 2. 离线全量放置使用
1. 下载 `BundleRRS.7z`（或精简版 `BundleRRS-lite.7z`）解压至 `~/.config/rkt/rulesets/` 目录。
2. 或下载单个 `geoip.rdb`（或 `geoip-lite.rdb`）放入 `~/.config/rkt/` 目录。
